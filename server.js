...
// ✅ تنظيف المفتاح الخاص وجعله نصاً متصلاً تماماً في سطر واحد بدون فراغات لتجاوز عيوب مكتبة Node-RSA الداخلية
const PRIVATE_KEY_PEM = "-----BEGIN RSA PRIVATE KEY-----MIIEogIBAAKCAQBj0PeaXtoumSrgkOTrhqf+D6EMy/glD/qoHoZYkjMkmT8skOca" +
"cK1DdITUKmozwuuW71GUHHGUttiwUEV+Yq33Dtk30H2zoPd4PjGDM3j4hsUFTrpI" +
...
"8xGpHTb68w2OFOxFEKyaXc9ADMWO+sMHTW4n0eMuXgisv4SQRDI=-----END RSA PRIVATE KEY-----";
...
// Route 1: جلب التوكن عبر استدعاء الـ SDK القياسي بعد كشف آليته الداخلية من ملفات الحزمة
app.post('/api/get-iframe-token', (req, res) => {
    console.log('Dispatching request wrapper for getTokenIframe...');

    // بناء المتغيرات الإضافية التي يتوقعها الرابط الفعلي كـ query params
    const queryParams = {
    ...
    LLPay.getTokenIframe({
        ...
        successcb: (result) => {
            ...
            // جلب التوكن من الـ body الداخلي للرد الفعلي للمكتبة
        ...
        failcb: (err) => {
            ...
            return res.status(400).json({ success: false, error: err || 'Token generation rejected' });
        }
    });
});
