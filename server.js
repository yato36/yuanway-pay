...
MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA4IuEpIVKAYsWgFPPwvEz
/nb6Ktz2RJwk1eImrNrn220ACM95Q8s2jLlIFM2CBfSwyPtyUJQxXOzMyKFIHOkd
Msw7ySFJD7H11u9ypklz3dduSrXcMUOArMsNAvhw0lhYWAaCqftsMa5giOIT7uU+
QLSRrKd4hUW+kJA6ATZ1xkmqUPkJ2mU5m6xGjgDfTUpxs9QoWU1ISg92ZBfZyDn
        ...
        successcb: (result) => {
            try {
                ...
                if (token) {
                    ...
                    return res.status(400).json({ success: false, error: parsed.return_message || 'No token', raw: parsed });
                }
            } catch (e) {
...
app.post('/api/execute-payment', (req, res) => {
    ...
    LLPay.pay({
        ...
        failcb: (err) => res.status(400).json({ success: false, error: err })
    });
});
...
app.listen(PORT, '0.0.0.0', () => console.log(`Yuanway Gateway listening on port ${PORT}`));